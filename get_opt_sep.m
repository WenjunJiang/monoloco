function [antenna_sep] = get_opt_sep(csi,antenna_type,init_sep,calib_const)
...
% antenna_type: string value, it should be "tx" if we calculate AoD, "rx"+N if we calculate AoA
...
[nSample,nTx,nRx,nChannel] = size(csi);
...
if any(strcmp(rAPs,antenna_type)) % calculate AoA
    ...
    mse_best = 1000000;
    for offset2 = -cc.max_offset:0.002:cc.max_offset
        for offset3 = -cc.max_offset:0.002:cc.max_offset
            rx2_1_gt = mod(-2*pi*(d2_1+offset2)*cos(aoa_target)*cc.f/cc.C+pi,2*pi)-pi;
            rx2_1_gt = repmat(rx2_1_gt',1,nTx,nSample);
            rx2_1_gt = permute(rx2_1_gt,[3,2,1]);
            
            rx3_2_gt = mod(-2*pi*(d3_2-offset2+offset3)*cos(aoa_target)*cc.f/cc.C+pi,2*pi)-pi;
            rx3_2_gt = repmat(rx3_2_gt',1,nTx,nSample);
            rx3_2_gt = permute(rx3_2_gt,[3,2,1]);
            
            rx3_1_gt = mod(-2*pi*(d3_1+offset2+offset3)*cos(aoa_target)*cc.f/cc.C+pi,2*pi)-pi;
            rx3_1_gt = repmat(rx3_1_gt',1,nTx,nSample);
            rx3_1_gt = permute(rx3_1_gt,[3,2,1]);
...
elseif any(strcmp(tAPs,antenna_type)) % calculate AoA
    ...
    for offset2 = -cc.max_offset:0.002:cc.max_offset
        for offset3 = -cc.max_offset:0.002:cc.max_offset
            tx2_1_gt = mod(-2*pi*(d2_1+offset2)*cos(aod_target)*cc.f/cc.C+pi,2*pi)-pi;
            tx2_1_gt = repmat(tx2_1_gt',1,nRx,nSample);
            tx2_1_gt = permute(tx2_1_gt,[3,2,1]);
            
            tx3_2_gt = mod(-2*pi*(d3_2-offset2+offset3)*cos(aod_target)*cc.f/cc.C+pi,2*pi)-pi;
            tx3_2_gt = repmat(tx3_2_gt',1,nRx,nSample);
            tx3_2_gt = permute(tx3_2_gt,[3,2,1]);
            
            tx3_1_gt = mod(-2*pi*(d3_1+offset2+offset3)*cos(aod_target)*cc.f/cc.C+pi,2*pi)-pi;
            tx3_1_gt = repmat(tx3_1_gt',1,nRx,nSample);
            tx3_1_gt = permute(tx3_1_gt,[3,2,1]);
