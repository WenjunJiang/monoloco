...
T = 100; F = 15; A = 3; AoD=1; L = 5; N = 100; FI = 2*312.5e3; TI = 1/sample_rate;
AS_vec = [0,0.009,0.028;...
          0,0.013,0.026;...
          0,0.009,0.015]; % 1D distance, num of rAP * num of antennas on each AP
DS_vec=[0];
...
needCalib = false;
musicType = 'TRAP'; %'RAP' or 'TRAP'

calib_const = calib_set_const(tx_loc,rx_loc,tx_orient,rx_orient,c,carrier_frequency,FI,max_offset,F,...
    x_range,y_range,x_step,y_step,AS_vec,DS_vec);
tic;

if needCalib
    for rAPId = [1,3]
        data_empty = load(strcat(data_path,'rx',num2str(rAPId),'_nobody_empty1.mat'));
        csi_data_empty=data_empty.csi_data(:,1:90);
        %% Data sanitization
...
%             end
%         end
        csi_data_empty = reshape(csi_data_empty',30,A,AoD,[]); 
%         csi_data_empty = permute(csi_data_empty,[4,3,2,1]); % reshape the data to nSample*nTx*nRx*nChannel
        csi_data_empty = csi_data_empty(1:F,:,:,:); %select channel
...
for fi = 1:length(motion_files)
    ...
    true_heatmap = cal_heatmap_groundtruth(person1_data,person2_data,calib_const);
...
%         aTot = gridVecBackscatter3D(ToFR,AoAR,AoDR,AS_vec(rAPId,:),DS_vec,calib_const,true); % Get steering vector of each possible combination
        aTot = xyVecBackscatter3D(strcat('rAP',num2str(rAPId)),calib_const,true);
        ...
        for i=1:lenSeg:size(csi_data,1)-lenSeg+1
            disp(i);
            toc;
            csi_data1 = reshape(csi_data(i:i+lenSeg-1,:)',30,A,AoD,[]); % reshape to nChannel*nRx*nTx*nSample
            csi_data1 = csi_data1(1:F,:,:,:);
            csi_data1 = reshape(csi_data1,F*A*AoD,lenSeg);
...
%             csi_data1 = csi_data(i:i+lenSeg-1,:)';% simplified version of "formatCSI",nChannel*nRx*nTx_nSample
            ...
            elseif strcmp(musicType,'TRAP')
                n_iter=1;
                music_spectrum = TRAPMusicGrid(aTot,csi_data1,n_iter,delayLoS,aoaLoS,aodLoS,AS_vec(rAPId,:),DS_vec,calib_const);
            end
